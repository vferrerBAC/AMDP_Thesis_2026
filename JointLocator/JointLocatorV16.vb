...
Sub Main()
    ...
    ' -----------------------------
    ' ✅ Workbook 1 → Joint Output
    ' -----------------------------
    Dim jointWorkbook As Object = excelApp.Workbooks.Add

    Dim sheet As Object = jointWorkbook.Sheets(1)
    sheet.Name = "Joints"

    Dim debugSheet As Object = jointWorkbook.Sheets.Add
    debugSheet.Name = "DebugLog"


    ' -----------------------------
    ' ✅ Workbook 2 → Capacity Template
    ' -----------------------------
    ...
    Dim capWorkbook As Object = excelApp.Workbooks.Open(capPath)

    ' MsgBox("Opened capacity file: " & capWorkbook.FullName)

    Dim boltedSheet As Object = capWorkbook.Sheets("Bolted Connections")
    Dim weldedSheet As Object = capWorkbook.Sheets("Welded Connections")
    ...
    sheet.Cells(1, 11).Value = "Connection Type"
    sheet.Cells(1, 12).Value = "Contact Area (in^2)"
    ...
    Dim boltedRow As Integer = 2
    Dim weldedRow As Integer = 2
        ...
        For j = i + 1 To compDef.Occurrences.Count
            ...
            For Each face1 As Face In body1.Faces
                ...
                For Each face2 As Face In body2.Faces
                    ...
                    Dim contactArea As Double = GetContactArea(face1, face2)
                    ...
                    Dim has1 As Boolean = HasFlatWidthIPT(occ1)
                    Dim has2 As Boolean = HasFlatWidthIPT(occ2)

                    Dim connectionType As String

                    If has1 And has2 Then
                        connectionType = "Bolted"
                    ElseIf Not has1 And Not has2 Then
                    ...
                    ' ----------------------------------------------------------
                    ' ✅ Populate Capacity Template
                    ' ----------------------------------------------------------
                    If connectionType = "Bolted" Then

                        Dim govOcc As ComponentOccurrence = GetGoverningOccurrence(occ1, occ2)

                        Dim gaugeVal As Double = GetGaugeValue(govOcc)
                        Dim matRaw As String = GetCustomiProperty(govOcc, "NCx_Material")
                        Dim matMapped As String = MapMaterial(matRaw)
                    ...
                    jointRows.Add(New Object() { _
                        ...
                        part1Name, mat1, part2Name, mat2, _
                        Round(jointLength, 4), Round(weldedJointLength, 4), _
                        connectionType, _
                        Round(contactArea, 4)})
    ...
    Dim saveFolderJointFile As String = "C:\Users\SRosario\OneDrive - BAC\Desktop\Joint Catalog\JointIdentifierOutputs"
    ' saveFolderCapFile As String = "C:\Users\SRosario\OneDrive - BAC\Desktop\Joint Catalog\StructuralAnalysis"
    ' -----------------------------
    ' ✅ Save Joint File
    ' -----------------------------
    ...
    ' capWorkbook.SaveAs(capPath)
    capWorkbook.Save()
    capWorkbook.Close(False)
...
' ==========================================================
' ✅ Check if Custom iProperty exists
' ==========================================================
Function HasFlatWidthIPT(occ As ComponentOccurrence) As Boolean
    ...
    Try
        ...
        ' Try to grab property
        Dim prop As [Property] = propSet.Item("CostDataFlatWidthInches")
...
Function GetCustomiProperty(occ As ComponentOccurrence, propName As String) As String
...
Function GetGaugeValue(occ As ComponentOccurrence) As Double

    Try
        Dim valStr As String = GetCustomiProperty(occ, "Gauge")
...
Function GetGoverningOccurrence(occ1 As ComponentOccurrence, occ2 As ComponentOccurrence) As ComponentOccurrence

    Dim g1 As Double = GetGaugeValue(occ1)
    Dim g2 As Double = GetGaugeValue(occ2)
...
Function MapMaterial(ncxMat As String) As String

    If ncxMat = "GLV-M5" Then
        Return "Galvanized Sheet Steel"
    Else
        Return ncxMat ' fallback
    End If
...
Function GetBoundingBoxDims(occ As ComponentOccurrence) As Double()
    ...
    Dim dx As Double = Math.Abs(box.MaxPoint.X - box.MinPoint.X) / 2.54
...
' ==========================================================
' ✅ Contact Area (in^2)
' Uses same polygon clipping as centroid/length
' ==========================================================
Function GetContactArea(face1 As Face, face2 As Face) As Double
    ...
    If obb1 Is Nothing Or obb2 Is Nothing Then Return 0
    ...
    Dim area_cm2 As Double = Math.Abs(SignedArea(clipped))

    ' Convert cm² → in²
    Dim area_in2 As Double = area_cm2 / (2.54 * 2.54)
