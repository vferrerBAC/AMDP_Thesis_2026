    ...
    for c in combos:
        ...
        for jid, items in touch.items():
            ...
            for ename, end, x in items:
                ...
                actions.append({"member": ename.split("__")[0], "element": ename,
                                "end": end, **_global_end_action(el, end, c)})
            jmf[c][jid] = rows
            jma[c][jid] = actions
    return {"reactions": reactions, "joint_member_forces": jmf,
            "joint_member_actions": jma}


def _global_end_action(el, end: str, combo: str) -> dict:
    """The action the MEMBER delivers into the joint node, global XYZ.

    PyNite's ``Member3D.F(combo)`` is the 12x1 global end-force vector: the force
    the NODES apply to the member (F = Ke @ T @ d). The connection at that node
    has to transmit the equal-and-opposite action the member applies to the node,
    so the vector is negated here. Verified against a pin-ended bar pulled in +X:
    F(j-end) reads +1000 while the member is in tension, i.e. the member pulls the
    node back with -1000.

    Downstream (``patch_groups.distribute``) depends on this sign: it is what makes
    a negative patch normal component mean the faces are being pulled APART.

    Units: lbf and lbf-in, matching the rest of the solver.
    """
    raw = el.F(combo)
    v = [float(x) for x in (raw.flatten() if hasattr(raw, "flatten") else raw)]
    o = 0 if end == "i" else 6
    return {"F": [_round(-v[o + 0]), _round(-v[o + 1]), _round(-v[o + 2])],
            "M": [_round(-v[o + 3]), _round(-v[o + 4]), _round(-v[o + 5])]}
