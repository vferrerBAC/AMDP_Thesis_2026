...
# Public alias — the standard gauge chart (BAC "Minimum Purchase Thickness"),
# used by the Stage 3 UI's Gauge column and Block 1 autopopulation.
GAUGE_THICKNESS_IN: Dict[int, float] = dict(_THICKNESS_CARBON_GALV)


def thickness_from_gauge(gauge: Any) -> Optional[float]:
    """Translate a sheet-metal gauge number (e.g. from the Inventor ``Gauge``
    iProperty) to minimum purchase thickness (in) via the standard gauge chart.
    Returns None for a missing/unrecognized gauge; callers should route that to
    review rather than guess a thickness."""
    try:
        return GAUGE_THICKNESS_IN.get(int(round(float(gauge))))
    except (TypeError, ValueError):
        return None


def governing_gauge(gauges: Any) -> "Optional[tuple[int, float]]":
    """Given gauges of the members meeting at a joint, return (gauge, thickness)
    for the one that governs the bolted-connection capacity: the SMALLEST
    resulting thickness (i.e. the thinnest / weakest connected part — note this
    is the LARGEST gauge number, since gauge numbering is inverse to thickness).
    Unrecognized gauges are ignored. Returns None if nothing resolves."""
    best = None
    for g in gauges:
...
def default_bolted_row(connection_id: Any, joint_id: Any = "") -> Dict[str, Any]:
    """Return a default connection-schedule row for a single CONNECTION.

    The schedule is keyed by ``connection_id`` because that is where the geometry
    a capacity equation needs actually lives (contact area, thickness, hole
    pattern). ``joint_id`` rides along so results can be rolled up to the joint
    for reporting — one joint can own several connections, each with its own
    thickness, bolt count, and material.
    """
    return {
