...
Sub Main()
    ...
    ' Calculation can only be accessed once a workbook is open.
    Dim originalCalc As Object = excelApp.Calculation
    ...
    For Each oOccurrence As ComponentOccurrence In oAsmDoc.ComponentDefinition.Occurrences
        ...
        If TryGetCustomiProp(oPartDoc, "CostDataFlatLengthInches", probe) Then
            ' MsgBox("SHEET METAL path: " & oPartDoc.DisplayName)
            data = BuildSheetMetalData(oPartDoc)
        Else
            ' MsgBox("TUBE path: " & oPartDoc.DisplayName)
    ...
    ' ===== SAVE + CLEANUP =====
    excelApp.Calculation = originalCalc   ' restore, forces recalc
